...
for forAmp = forcingAmplitudes
    ...
    figure(1000);
    hold on 
    h = plot([freqsLow freqsHigh(2:end)],[ampsLow ampsHigh(2:end)],'.k','MarkerSize',12,'HandleVisibility','off');
    drawnow;
    delete(h)
    plot([freqsLow freqsHigh(2:end)],[ampsLow ampsHigh(2:end)],'.k','MarkerSize',12,'DisplayName','Full Model');
...
for forAmp = forcingAmplitudes
    ...
    forFreq = detunings(2);
    % Initial condition
%     x0 = real((1i*forFreq*eye(2*n)-A)\forcingVectors_na)*forAmp;
    ampsLin = (K-eye(n)*forFreq^2)\forcingVectors_na(n+1:end)*forAmp;
    x0 = [ampsLin; zeros(n,1)];
    timeSpan = [0 nPers*2*pi/forFreq];
    % Integration
...
%           end              
    ...
    figure(1000);
    hold on 
    h = plot(freqsLow,ampsLow,'.k','MarkerSize',12,'HandleVisibility','off');
    drawnow;
    delete(h)
    plot(freqsLow,ampsLow,'.k','MarkerSize',12,'DisplayName','Full Model');
...
%% impactless FRC by means of SSMTool

K_delta = zeros(n); K_delta(outdof, outdof) = delta; 

forcingAmplitudes = [25 35];
omegaSpan = ( abs(imag(lambda(1))) + abs(imag(lambda_minus(1))))/2 * [0.9 1.1];
ROMOrder = 7;
FRCSSMTool_data = cell(1,length(forcingAmplitudes));
for ii = 1:length(forcingAmplitudes)
    fFull = forcingAmplitudes(ii);    
    FRCSSMTool = SSMToolFRCcustom(M, C, K, fnl, fExt, fFull, outdof, omegaSpan, ROMOrder,2e3); 
    FRCSSMTool_data{1,ii} = FRCSSMTool;
end
close all
...
colors = colororder;
for ii = 1:length(forcingAmplitudes)
    plotFRC(FRCSSMTool_data{1,ii}, colors(2,:), '- SSMTool')
end
legend('impactless stable - SSMTool','impactless unstable - SSMTool')
xlim([omegaSpan(1) omegaSpan(2)]);
ylim([0 1e-2]);

save('frc_wofriction', 'FRCSSMTool_data','omegaSpan');
